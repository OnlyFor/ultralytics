...
class ObjectCounter:
    ...
    def __init__(self, **args):
        ...
        # Region & Line Information + initialize counting region
    ...
    def start_counting(self, im0):
        ...
        """
        annotator = Annotator(im0, DEFAULT_CFG_DICT["line_width"], self.model.names)

        # Draw region or line
        annotator.draw_region(reg_pts=self.CFG["region"], color=(104, 0, 123), thickness=DEFAULT_CFG_DICT["line_width"] * 2)
        ...
        if tracks[0].boxes.id is not None:
            ...
            for box, track_id, cls in zip(boxes, track_ids, clss):
                # Draw bounding box
                annotator.box_label(box, label=self.model.names[cls], color=colors(int(track_id), True))
        ...
        if DEFAULT_CFG_DICT["show"] and self.env_check:
            cv2.imshow("Ultralytics YOLOv8 Object Counter", im0)
